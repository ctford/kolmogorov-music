...
He starts by assuming that musical analysis aims to produce "best possible explanation for musical works". If that's the case,
...
Representing the notes of a piece as code is straightforward, as a series of notes can, to a first approximation,
be generated from a sequence of durations and a sequence of pitches. The following example uses Leipzig's \verb|phrase| function,
and produces a sequence of time/pitch/duration tuples.
...
vast fictional library, thousands of hexagonal rooms house all possible books of four hundred and ten pages. Melancholic
librarians spend their entire lives toiling in its maddiningly symetrical depths.
...
A lexicon contains every finite string as a substring. It's called a lexicon because it says everything that is possible
...
concisely yet contain multitudes. Paradoxically, the whole is simpler than (almost all of) the parts.
...
epitomises the complexity of the entire library, though it is part of the library.
...
of algorithmic creation, but it would be difficult to analyse 'Contact' and 'The Library of Babel' through Kolmogorov's lens.
...
The encoding is relatively simple, each four digit word in the natural number is turned into a note, two digits for duration and
